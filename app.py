...
# --- DATABASE TEMPORANEO (In un'app reale useresti un file CSV o un DB) ---
...
def calcola_parametri(materiale, giunto, spessore_target):
    ...
    # La potenza segue una curva di dissipazione (esponente 1.15 per Raycus 2kW)
    potenza = base['Potenza'] * (ratio ** 1.15) * moltiplicatori[giunto]
    wobble_w = min(base['Wobble_W'] * (ratio ** 0.4), 4.5) # Limite torcia WSX
...
menu = ["Calcolatore Operatore", "Area Amministratore"]
...
if choice == "Calcolatore Operatore":
    ...
    if st.button("Genera Parametri"):
        ...
        if res:
            ...
            st.error("Nessun dato base trovato per questo materiale.")

elif choice == "Area Amministratore":
    st.header("⚙️ Configurazione Sistema")
    st.write("Inserisci qui i parametri che hai testato con successo per migliorare l'algoritmo.")
    
    with st.form("admin_form"):
        new_mat = st.text_input("Materiale (es. Acciaio Inox)")
        new_giunto = st.selectbox("Giunto di test", ["Butt Joint", "Full penetration", "Inner Joint", "Outer Joint"])
        new_spessore = st.number_input("Spessore (mm)", value=1.0)
        new_potenza = st.number_input("Potenza Reale (W)", value=450)
        ...
        if st.form_submit_button("Salva nel Database"):
            nuovo_dato = {
                ...
                "Potenza": new_potenza, "Wobble_W": new_w_w, "Wobble_F": new_w_f, "V_Filo": new_v_f
            }
